...
# model.eval()  when loading model for evaluation
model.train()
...
def loadHyperParamResult(filename):
    if os.path.isfile(filename):
        ...
        trL = checkpoint['TrLossPerEpoch'] 
        trL[0] = trL[0]*100
        valL = checkpoint['ValLossPerEpoch']
        print(checkpoint)
        return curLosses, curLR, curBS, trL, valL
    else:
        print("reeeedim!!")
...
###############################################################
def plotResult(trainingLoss, validationLoss, curLR, curBS):
    xx = []
    for i in range(17):
        xx.append((i+1)*100)
    plt.figure(0)
    plt.plot(xx, trainingLoss, label="Training loss")
    plt.plot(xx, validationLoss, label="Validation loss")
    plt.ylabel('Loss')
    plt.xlabel('Batch')
